...
const TString sPlotsStore = "plotsXSRatio/";
...
void cmsPreliminary(TCanvas* &c){
    ...
    CMSPrelim->DrawLatex(0.93, 0.96,"19.7 fb^{-1} at #sqrt{s} = 8 TeV");
...
void setCavasAndStyles(TString canvasName, TCanvas* &c, TString stat = "", double leftMaring = 0.15, double rightMaring = 0.05, double bottomMaring = 0.15, double topMaring = 0.05){
    // setup environment
    gROOT->ProcessLine(".L setTDRStyle.C"); setTDRStyle(true);
    // setup canvas
...
void plotXSTemplatesRatio() {
    ...
    TString obsTag = "mass4l";
    const TString sTemplateDirName = "templatesXSRatio/DTreeXS_"+obsTag+"/8TeV/";
    const int N_BINS = 1;
    TString binRange[N_BINS]     = {"50_140"};
    TString binRangeLow[N_BINS]  = {"50"};
    TString binRangeHigh[N_BINS] = {"140"};
    TString binRangeLeg[N_BINS]  = {"50 < m(4l) < 140 GeV"};

//    TString obsTag = "pT4l";
//    const TString sTemplateDirName = "templatesXSRatio/DTreeXS_"+obsTag+"/8TeV/";
//    const int N_BINS = 4;
//    TString binRange[N_BINS]     = {"0_15",      "15_30",      "30_60",      "60_1000"};
//    TString binRangeLow[N_BINS]  = {"0",      "15",      "30",      "60"};
//    TString binRangeHigh[N_BINS] = {"15",      "30",      "60",      "1000"};
//    TString binRangeLeg[N_BINS]  = {"0 < p_{T} < 15 GeV",      "15 < p_{T} < 30 GeV",      "30 < p_{T} < 60 GeV",      "p_{T} > 60 GeV"};

//    TString obsTag = "massZ2";
//    const TString sTemplateDirName = "templatesXS/DTreeXS_"+obsTag+"/8TeV/";
//    const int N_BINS = 4;
...
//    TString obsTag = "njets_reco_pt30_eta4p7";
//    const TString sTemplateDirName = "templatesXS/DTreeXS_"+obsTag+"/8TeV/";
//    const int N_BINS = 4;
//    TString binRange[N_BINS]   = {"0_1",      "1_2",      "2_3",      "3_10"};
...
//                for (int k = 0; k < nSmooth; k++) smoothAndNormaliseTemplate1D(h1D_4e[iBkg][iBin]);
    ...
    // prepare dummy
    double var_plotHigh = 140, var_plotLow = 50;
    int var_nBins = 30;
    ...
    for (int iBin = 0; iBin<N_BINS; iBin++ ) {
        ...
        leg->Draw(); c1->SaveAs(sPlotsStore+"/XSTemplates_2e2mu_"+obsTag+"_"+binRange[iBin]+"_"+bkgName[kBkg_qqZZ]+"_"+bkgName[kBkg_ggZZ]+".pdf");
        // qqZZZ + Z+X //
        h1D_dummy->SetMaximum(2.0*h1D_2e2mu[kBkg_qqZZ][iBin]->GetMaximum());
        h1D_dummy->Draw(); cmsPreliminary(c1); TLegend* leg = new TLegend(leg_xl,leg_yb,leg_xr,leg_yt); setLegendProperties(leg,binRangeLeg[iBin]+", 2e2#mu");
        setHistProperties(h1D_2e2mu[kBkg_qqZZ][iBin],lineWidth,1,kBlack); h1D_2e2mu[kBkg_qqZZ][iBin]->Draw("same"); leg->AddEntry(h1D_2e2mu[kBkg_qqZZ][iBin], "q#bar{q} #rightarrow ZZ","L");
        ...
        leg->Draw(); c1->SaveAs(sPlotsStore+"/XSTemplates_2e2mu_"+obsTag+"_"+binRange[iBin]+"_"+bkgName[kBkg_qqZZ]+"_"+bkgName[kBkg_ZJets]+".pdf");

    std::cout << "here iBin=" << iBin << std::endl;
        ...
        leg->Draw(); c1->SaveAs(sPlotsStore+"/XSTemplates_4mu_"+obsTag+"_"+binRange[iBin]+"_"+bkgName[kBkg_qqZZ]+"_"+bkgName[kBkg_ggZZ]+".pdf");
        // qqZZZ + Z+X //
        h1D_dummy->SetMaximum(2.0*h1D_4mu[kBkg_qqZZ][iBin]->GetMaximum());
        h1D_dummy->Draw(); cmsPreliminary(c1); TLegend* leg = new TLegend(leg_xl,leg_yb,leg_xr,leg_yt); setLegendProperties(leg,binRangeLeg[iBin]+", 4#mu");
        setHistProperties(h1D_4mu[kBkg_qqZZ][iBin],lineWidth,1,kBlack); h1D_4mu[kBkg_qqZZ][iBin]->Draw("same"); leg->AddEntry(h1D_4mu[kBkg_qqZZ][iBin], "q#bar{q} #rightarrow ZZ","L");
        ...
        leg->Draw(); c1->SaveAs(sPlotsStore+"/XSTemplates_4mu_"+obsTag+"_"+binRange[iBin]+"_"+bkgName[kBkg_qqZZ]+"_"+bkgName[kBkg_ZJets]+".pdf");
        ...
        leg->Draw(); c1->SaveAs(sPlotsStore+"/XSTemplates_4e_"+obsTag+"_"+binRange[iBin]+"_"+bkgName[kBkg_qqZZ]+"_"+bkgName[kBkg_ggZZ]+".pdf");
        // qqZZZ + Z+X //
        h1D_dummy->SetMaximum(2.0*h1D_4e[kBkg_qqZZ][iBin]->GetMaximum());
        h1D_dummy->Draw(); cmsPreliminary(c1); TLegend* leg = new TLegend(leg_xl,leg_yb,leg_xr,leg_yt); setLegendProperties(leg,binRangeLeg[iBin]+", 4e");
        setHistProperties(h1D_4e[kBkg_qqZZ][iBin],lineWidth,1,kBlack); h1D_4e[kBkg_qqZZ][iBin]->Draw("same"); leg->AddEntry(h1D_4e[kBkg_qqZZ][iBin], "q#bar{q} #rightarrow ZZ","L");
        ...
        leg->Draw(); c1->SaveAs(sPlotsStore+"/XSTemplates_4e_"+obsTag+"_"+binRange[iBin]+"_"+bkgName[kBkg_qqZZ]+"_"+bkgName[kBkg_ZJets]+".pdf");

    } // iBin<N_BINS


    std::cout << "here Z+X all bins" << std::endl;
    ...
    if (N_BINS>1) { setHistProperties(h1D_2e2mu[kBkg_ZJets][1],lineWidth,2,kRed-7); h1D_2e2mu[kBkg_ZJets][1]->Draw("same"); leg->AddEntry(h1D_2e2mu[kBkg_ZJets][1], "Z + X, 1","L"); }
    if (N_BINS>2) { setHistProperties(h1D_2e2mu[kBkg_ZJets][2],lineWidth,3,kRed-7); h1D_2e2mu[kBkg_ZJets][2]->Draw("same"); leg->AddEntry(h1D_2e2mu[kBkg_ZJets][2], "Z + X, 2","L");}
    if (N_BINS>3) { setHistProperties(h1D_2e2mu[kBkg_ZJets][3],lineWidth,4,kRed-7); h1D_2e2mu[kBkg_ZJets][3]->Draw("same"); leg->AddEntry(h1D_2e2mu[kBkg_ZJets][3], "Z + X, 3","L");}
    TH1D* hSum = (TH1D*) h1D_2e2mu[kBkg_ZJets][0]->Clone(); 
    if (N_BINS>1) hSum->Add(h1D_2e2mu[kBkg_ZJets][1]);
    if (N_BINS>2) hSum->Add(h1D_2e2mu[kBkg_ZJets][2]); 
    if (N_BINS>3) hSum->Add(h1D_2e2mu[kBkg_ZJets][3]);
    normaliseHist1D(hSum);
    setHistProperties(hSum,lineWidth,1,kBlack); hSum->Draw("same"); leg->AddEntry(hSum, "Z + X, all","L");
    leg->Draw(); c1->SaveAs(sPlotsStore+"/XSTemplates_AllChans_"+obsTag+"_"+bkgName[kBkg_ZJets]+"_allBins.pdf");

    std::cout << "here qqZZ all bins" << std::endl;
    ...
    TH1D* hSum_2e2mu = (TH1D*) h1D_2e2mu[kBkg_qqZZ][0]->Clone(); 
    if (N_BINS>1) hSum_2e2mu->Add(h1D_2e2mu[kBkg_qqZZ][1]); 
    if (N_BINS>2) hSum_2e2mu->Add(h1D_2e2mu[kBkg_qqZZ][2]); 
    if (N_BINS>3) hSum_2e2mu->Add(h1D_2e2mu[kBkg_qqZZ][3]);
    TH1D* hSum_4mu = (TH1D*) h1D_4mu[kBkg_qqZZ][0]->Clone(); 
    if (N_BINS>1) hSum_4mu->Add(h1D_4mu[kBkg_qqZZ][1]); 
    if (N_BINS>2) hSum_4mu->Add(h1D_4mu[kBkg_qqZZ][2]); 
    if (N_BINS>3) hSum_4mu->Add(h1D_4mu[kBkg_qqZZ][3]);
    TH1D* hSum_4e = (TH1D*) h1D_4e[kBkg_qqZZ][0]->Clone(); 
    if (N_BINS>1) hSum_4e->Add(h1D_4e[kBkg_qqZZ][1]); 
    if (N_BINS>2) hSum_4e->Add(h1D_4e[kBkg_qqZZ][2]); 
    if (N_BINS>3) hSum_4e->Add(h1D_4e[kBkg_qqZZ][3]);
    ...
    leg->Draw(); c1->SaveAs(sPlotsStore+"/XSTemplates_AllChans_"+obsTag+"_"+bkgName[kBkg_qqZZ]+"_allBins.pdf");

...
void templatesXS(TString processNameTag, TString processFileName, TString sqrtsTag, TString sfinalState,
                 TString obsName, TString obsBinDn, TString obsBinUp);
...
int nbinsX=20; const int nbinsY=20;
...
void fiducialXSTemplates(TString processNameTag = "qqZZ", TString processFileName = "ZZTo2e2mu_mZZ95-160.root", TString sfinalState = "4l", TString obsName = "massZ2", TString obsBinDn = "0", TString obsBinUp = "120", TString sqrtsTag = "8TeV", TString baseDirXS = "templatesXS", TString sProcessingType = "DTreeXS", TString doZ4l = "false", TString doRatio = "false"){
    ...
    templatesXS(processNameTag, processFileName, sqrtsTag, sfinalState, obsName, obsBinDn, obsBinUp);
}
...
void templatesXS(TString processNameTag, TString processFileName, TString sqrtsTag, TString sfinalState,
    ...
    if (sfinalState == "4l") {
        getTemplateXS(processNameTag, processFileName, sqrtsTag, "2e2mu", obsName, obsBinDn, obsBinUp);
        getTemplateXS(processNameTag, processFileName, sqrtsTag, "4e", obsName, obsBinDn, obsBinUp);
        getTemplateXS(processNameTag, processFileName, sqrtsTag, "4mu", obsName, obsBinDn, obsBinUp);
    } else {                            
        getTemplateXS(processNameTag, processFileName, sqrtsTag, sfinalState, obsName, obsBinDn, obsBinUp);
    }
}
...
int getHistTreesXS(TChain* tree, TString processNameTag, TString sqrtsTag, TTree* TT,
                   ...
                   int iFinState, double ptElCut, double ptMuCut, double mZ2Cut, double mZ1Cut, double m4l_low, double m4l_high, bool scale = false){
    // define vars
    int idL1, idL2, idL3, idL4;
    double pTL1, pTL2, pTL3, pTL4;
    double etaL1, etaL2, etaL3, etaL4;
    double mass4l, pT4l, eta4l, massZ1, massZ2;//, MEKD;
    int nJets;
    int njets_reco_pt30_eta4p7=0, njets_reco_pt30_eta4p7_jesdn=0, njets_reco_pt30_eta4p7_jesup=0;
    ...
    if (tree->GetBranch("dataMC_weight")) {tree->SetBranchAddress("dataMC_weight",&dataMC_weight);}
    ...
    if (tree->GetBranch("njets_reco_pt30_eta4p7")) {tree->SetBranchAddress("njets_reco_pt30_eta4p7",&njets_reco_pt30_eta4p7);}
    if (tree->GetBranch("njets_reco_pt30_eta4p7_jesdn")) {tree->SetBranchAddress("njets_reco_pt30_eta4p7_jesdn",&njets_reco_pt30_eta4p7_jesdn);}
    if (tree->GetBranch("njets_reco_pt30_eta4p7_jesup")) {tree->SetBranchAddress("njets_reco_pt30_eta4p7_jesup",&njets_reco_pt30_eta4p7_jesup);}
    ...
    TT->Branch("nJets",&nJets,"nJets/I");
    TT->Branch("njets_reco_pt30_eta4p7",&njets_reco_pt30_eta4p7,"njets_reco_pt30_eta4p7/I");
    TT->Branch("njets_reco_pt30_eta4p7_jesup",&njets_reco_pt30_eta4p7_jesup,"njets_reco_pt30_eta4p7_jesup/I");
    TT->Branch("njets_reco_pt30_eta4p7_jesdn",&njets_reco_pt30_eta4p7_jesdn,"njets_reco_pt30_eta4p7_jesdn/I");
    ...
    TT->Branch("pTZ1",&pTZ1,"pTZ1/D");
    TT->Branch("pTZ2",&pTZ2,"pTZ2/D");
    ...
    TT->Branch("kfactor_ggZZ",&kfactor_ggZZ,"kfactor_ggZZ/D");
...
//    nentries = 1000;
    if(!SILENT) {
        ...
        cout << "Event" << ":" << "Run" << ":" << "LumiSect" << ":" << "mass4l" << ":" << "massZ1" << ":" << "massZ2" << ":" << "weight" << ":" << "kd_ALT" << ":" << "superKD" << ":" << "pdfSigM4l" << ":" << "pdfBkgM4l" << endl;
    }
...
//        if(iEvt%50000==0) cout << "   event: " << iEvt << "/" << nentries << endl;
        ...
        // weight
        if (dataMC_weight==0) dataMC_weight = 1;
        double weight = (scale)? eventWeight*scaleWeight*dataMC_weight: 1.;

        // selection - Matt
        if (!passedFullSelection && (processNameTag != "ZJetsCR")) continue;
        if (!passedQCDcut) continue;
        // selection - explicit
        if( abs(idL1) == 13 && (pTL1 < ptMuCut || abs(etaL1) > etaMuCut) ) continue;
        if( abs(idL2) == 13 && (pTL2 < ptMuCut || abs(etaL2) > etaMuCut) ) continue;
        if( abs(idL3) == 13 && (pTL3 < ptMuCut || abs(etaL3) > etaMuCut) ) continue;
        if( abs(idL4) == 13 && (pTL4 < ptMuCut || abs(etaL4) > etaMuCut) ) continue;
        if( abs(idL1) == 11 && (pTL1 < ptElCut || abs(etaL1) > etaElCut) ) continue;
        if( abs(idL2) == 11 && (pTL2 < ptElCut || abs(etaL2) > etaElCut) ) continue;
        if( abs(idL3) == 11 && (pTL3 < ptElCut || abs(etaL3) > etaElCut) ) continue;
        if( abs(idL4) == 11 && (pTL4 < ptElCut || abs(etaL4) > etaElCut) ) continue;
        if (((idL1+idL2+idL3+idL4)!=0) || massZ1<mZ1Cut || massZ1>120 || massZ2<mZ2Cut || massZ2>120 || mass4l<m4l_low || mass4l>m4l_high) continue;
        ...
        if (mass4l <= 100) kfactor_ggZZ = 1.0;
...
int getTemplateXS(TString processNameTag, TString processFileName, TString sqrtsTag, TString sfinalState,
    ...
    if (processNameTag == "Data") {
        sigTree = new TChain("AnaAfterHlt/passedEvents");
    } else if (processNameTag == "ZJetsCR") {
        sigTree = new TChain("AnaAfterHlt/passedEvents_ZX");
//        sigTree = new TChain("AnaAfterHlt/passedEvents");
        loadFakeRateHists();
    } else {
//        sigTree = new TChain("passedEvents_dataMC");
        sigTree = new TChain("AnaAfterHlt/passedEvents");
    }
    ...
    getHistTreesXS(sigTree, processNameTag, sqrtsTag, TT, h2D_m4l_mZ2, h2D_m4l_pT4l, h2D_m4l_eta4l, h2D_m4l_nJets,
                   iFinState, elpT, mupT, mZ2_low, mZ1_low, m4l_low, m4l_high, true);
...
void storeTreeAndTemplatesXS(TTree* TT, TString obsName, TString obsBinDn, TString obsBinUp, TString sfinalState, TString fLocation, TString templateNameTag, TString fOption){
    ...
    for (int k = 0; k < 5; k++) smoothAndNormaliseTemplate1D(h1D); // 5-smoothing

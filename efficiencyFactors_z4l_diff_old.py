...
def geteffs_z4l(channel, List, m4l_bins, m4l_low, m4l_high, obs_reco, obs_gen, obs_bins, recobin, genbin):    
    ...
    for Sample in List:
        ...
        Histos[processBin+"recoz4l_inc"] = TH1D(processBin+"recoz4l_inc", processBin+"recoz4l_inc", m4l_bins, m4l_low, m4l_high)
        Histos[processBin+"recoz4l_inc"].Sumw2() 
        ...
        Histos[processBin+"reconotz4l_inc"] = TH1D(processBin+"reconotz4l_inc", processBin+"reconotz4l_inc", m4l_bins, m4l_low, m4l_high)
        Histos[processBin+"reconotz4l_inc"].Sumw2()                 
        if (("jet" in opt.OBSNAME) or ("Jet" in opt.OBSNAME)):
        ...
        Histos[processBin+"anyrecoz4lfid"] = TH1D(processBin+"anyrecoz4lfid", processBin+"anyrecoz4lfid", m4l_bins, m4l_low, m4l_high)
        Histos[processBin+"anyrecoz4lfid"].Sumw2()                
        ...
        Histos[processBin+"recoz4lnotfid_inc"] = TH1D(processBin+"recoz4lnotfid_inc", processBin+"recoz4lnotfid_inc", m4l_bins, m4l_low, m4l_high)
        Histos[processBin+"recoz4lnotfid_inc"].Sumw2() 
        ...
        TreesPassedEvents[Sample].Draw("mass4l >> "+processBin+"recoz4l_inc","("+recoweight+")*("+cutm4l_reco+" && passedFullSelection==1 && "+cutz4l_reco+")","goff")
        TreesPassedEvents[Sample].Draw("mass4l >> "+processBin+"reconotz4l","("+recoweight+")*("+cutm4l_reco+" && "+cutobs_reco+" && passedFullSelection==1 && "+cutnotz4l_reco+")","goff")  
        TreesPassedEvents[Sample].Draw("mass4l >> "+processBin+"reconotz4l_inc","("+recoweight+")*("+cutm4l_reco+" && passedFullSelection==1 && "+cutnotz4l_reco+")","goff") 
        ...
        TreesPassedEvents[Sample].Draw("mass4l >> "+processBin+"recoz4lnotfid_inc","("+recoweight+")*("+cutm4l_reco+" && passedFullSelection==1 && "+cutz4l_reco+" && "+cutchan_gen_out+" && (passedFiducialTopology==0 || !("+cutz4l_gen+") || !("+cutm4l_gen+")) )","goff") 
        ...
        TreesPassedEvents[Sample].Draw("mass4l >> "+processBin+"anyrecoz4lfid","("+recoweight+")*("+cutm4l_reco+" && passedFullSelection==1 && "+cutz4l_reco+" && passedFiducialTopology==1 && "+cutz4l_gen+" && "+cutm4l_gen+" && "+cutchan_gen+" && "+cutobs_gen+")","goff")
        ...
        if (Histos[processBin+"reco_inc"].Integral()>0):
            wrongfrac[processBin] = Histos[processBin+"reconotz4l_inc"].Integral()/Histos[processBin+"reco_inc"].Integral()
        ...
        if (Histos[processBin+"reconotz4l_inc"].Integral()>0):
            binfrac_wrongfrac[processBin] = Histos[processBin+"reconotz4l"].Integral()/Histos[processBin+"reconotz4l_inc"].Integral()
            dbinfrac_wrongfrac[processBin] = sqrt(binfrac_wrongfrac[processBin]*(1-binfrac_wrongfrac[processBin])/Histos[processBin+"reconotz4l_inc"].Integral())
        else:
            binfrac_wrongfrac[processBin] = -1.0
            dbinfrac_wrongfrac[processBin] = -1.0
            
        if (Histos[processBin+"recoz4l_inc"].Integral()>0):
            outfrac[processBin] = Histos[processBin+"recoz4lnotfid_inc"].Integral()/Histos[processBin+"recoz4l_inc"].Integral()
            doutfrac[processBin] = sqrt(outfrac[processBin]*(1-outfrac[processBin])/Histos[processBin+"recoz4l_inc"].Integral())
        else:
            outfrac[processBin] =  -1.0
            doutfrac[processBin] = -1.0

        if (Histos[processBin+"recoz4lnotfid_inc"].Integral()>0):
            binfrac_outfrac[processBin] = Histos[processBin+"recoz4lnotfid"].Integral()/Histos[processBin+"recoz4lnotfid_inc"].Integral()
            dbinfrac_outfrac[processBin] = sqrt(binfrac_outfrac[processBin]*(1-binfrac_outfrac[processBin])/Histos[processBin+"recoz4lnotfid_inc"].Integral())
        else:
            binfrac_outfrac[processBin] =  -1.0
            dbinfrac_outfrac[processBin] = -1.0                                                                       

        if (Histos[processBin+"fid"].Integral()>=10.0):
            effanyreco[processBin] =  Histos[processBin+"anyrecoz4lfid"].Integral()/Histos[processBin+"fid"].Integral()
            ...
            cfactor[processBin] = Histos[processBin+"recoz4l"].Integral()/Histos[processBin+"fid"].Integral()
        else:
            effanyreco[processBin] = -1.0
            deffanyreco[processBin] = -1.0
            effrecotofid[processBin] = -1.0
            deffrecotofid[processBin] = -1.0
            cfactor[processBin] = Histos[processBin+"recoz4l"].Integral()/1.0 # if N(fid) for a gen bin is 0.0, change it to 1.0
            #dcfactor[processBin] = -1.0

        if (Histos[processBin+"anyrecoz4lfid"].Integral()>0.0):
            folding[processBin] = Histos[processBin+"recoz4lfid"].Integral()/Histos[processBin+"anyrecoz4lfid"].Integral()
            dfolding[processBin] = sqrt(folding[processBin]*(1-folding[processBin])/Histos[processBin+"anyrecoz4lfid"].Integral())
        else:
        ...
        if ((Histos[processBin+"recoz4lfid"].Integral()+Histos[processBin+"recoz4lotherfid"].Integral())>0.0):
            ...
            if (Histos[processBin+"recoz4lnotfid"].Integral()>0):
                doutinratio[processBin] = outinratio[processBin]*sqrt(1.0/(Histos[processBin+"recoz4lnotfid"].Integral())+1.0/(Histos[processBin+"recoz4lfid"].Integral()+Histos[processBin+"recoz4lotherfid"].Integral()))
        ...
        if (doFit): 
            ...
            CMS_zz4l_sigma_sig = RooRealVar("CMS_zz4l_sigma_sig","CMS_zz4l_sigma_sig",0.0,-0.2,0.02);
        ...
        if (doPlots):
            ...
            Histos[processBin+"reconotz4l"].SetFillColor(0)
            Histos[processBin+"reconotz4l"].SetLineColor(kOrange)
            hs.Add(Histos[processBin+"reconotz4l"])
            Histos[processBin+"recoz4lotherfid"].SetFillColor(0)
            Histos[processBin+"recoz4lotherfid"].SetLineColor(kBlue)
            hs.Add(Histos[processBin+"recoz4lotherfid"])
            Histos[processBin+"recoz4lnotfid"].SetFillColor(0)
            Histos[processBin+"recoz4lnotfid"].SetLineColor(kBlack)
            hs.Add(Histos[processBin+"recoz4lnotfid"])                                    
            Histos[processBin+"recoz4lfid"].SetFillColor(0)
            Histos[processBin+"recoz4lfid"].SetLineColor(kRed)
            hs.Add(Histos[processBin+"recoz4lfid"])

            leg = TLegend(0.54,0.57,0.91,0.72);
            leg.SetShadowColor(0)
            leg.SetFillColor(0)
            leg.SetLineColor(0)
            leg.AddEntry(Histos[processBin+"recoz4lfid"],"N_{sig}^{MC} = "+str(int(n_truesig)), "F")
            leg.AddEntry(Histos[processBin+"reconotz4l"],"N_{wrong}^{MC} = "+str(int(n_wrongsig)), "F")
...
for chan in chans:
    for recobin in range(len(obs_bins)-1):
        for genbin in range(len(obs_bins)-1):
            #List =  ['ZZTo'+chan+'_8TeV-powheg-pythia6','ZZTo'+chan+'_8TeV-powheg-pythia6_mll1_tchan']
            List =  ['ZZTo'+chan+'_8TeV-powheg-pythia6']
            geteffs_z4l(chan,List, m4l_bins, m4l_low, m4l_high, obs_reco, obs_gen, obs_bins, recobin, genbin)  
...
with open ('datacardInputs/moreinputs_sig_z4l_'+opt.OBSNAME+'.py', 'w') as f:

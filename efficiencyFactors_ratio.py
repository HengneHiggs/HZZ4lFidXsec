...
if (not os.path.exists("plots_ratio") and doPlots):
...
def plot_z4l(channel):
    ...
    for process in ['qqZZst', 'qqZZtchan']:
        ...
        c.SaveAs("plots_ratio/"+processBin+"_effs_"+recoweight+Tag+".png")
        c.SaveAs("plots_ratio/"+processBin+"_effs_"+recoweight+Tag+".pdf")
        c.SaveAs("plots_ratio/"+processBin+"_effs_"+recoweight+Tag+".C")

    #plot qqZZst and qqZZtchan together
    qqZZstBin = 'qqZZst_'+channel+'_mass4l_genbin0_recobin0'
    qqZZtchanBin = 'qqZZtchan_'+channel+'_mass4l_genbin0_recobin0'
    Histos[qqZZstBin+"recoz4l"].SetLineColor(kRed)
    Histos[qqZZstBin+"recoz4l"].SetMarkerColor(kRed)
    Histos[qqZZstBin+"recoz4l"].SetLineWidth(2)
    Histos[qqZZstBin+"recoz4l"].SetFillStyle(3216)
    Histos[qqZZstBin+"recoz4l"].SetFillColor(ROOT.kMagenta)
    Histos[qqZZtchanBin+"recoz4l"].SetLineColor(kBlue)
    Histos[qqZZtchanBin+"recoz4l"].SetMarkerColor(kBlue)
    Histos[qqZZtchanBin+"recoz4l"].SetLineWidth(2)
    Histos[qqZZtchanBin+"recoz4l"].SetFillStyle(3218)
    Histos[qqZZtchanBin+"recoz4l"].SetFillColor(ROOT.kCyan)
    lg = TLegend(0.57,0.6,0.86,0.8);
    lg.SetShadowColor(0)
    lg.SetFillColor(0)
    lg.SetLineColor(0)
    lg.AddEntry(Histos[qqZZstBin+"recoz4l"],"qqZZ (s+t)", "pl")
    lg.AddEntry(Histos[qqZZtchanBin+"recoz4l"],"qqZZ (t)", "pl")
    # 
    c = TCanvas("c","c",750,750)
    SetOwnership(c,False)
    c.cd()
    ...
    Histos[qqZZstBin+"recoz4l"].Draw("hist")
    Histos[qqZZtchanBin+"recoz4l"].Draw("hist same")
    lg.Draw("same")
    latex2 = TLatex()
    latex2.SetNDC()
    latex2.SetTextSize(0.75*c.GetTopMargin())
    ...
    c.SaveAs("plots_ratio/qqZZst_qqZZtchan_"+channel+"_"+recoweight+Tag+".png")
    c.SaveAs("plots_ratio/qqZZst_qqZZtchan_"+channel+"_"+recoweight+Tag+".pdf") 
    c.SaveAs("plots_ratio/qqZZst_qqZZtchan_"+channel+"_"+recoweight+Tag+".C") 
...
with open ('datacardInputs/inputs_sig_ratio_z4l_mass4l'+Tag+'.py', 'w') as f:
...
recoweight = RecoWeight
#write fout
fout = TFile("plots_ratio/qqZZst_qqZZtchan_"+recoweight+Tag+".root","recreate");
for chan in ['4e','4mu','2e2mu']:
    for process in ['qqZZst','qqZZtchan']:
